...
import { buildPathGraphData } from '../utils/graphUtils';
...
export default function CharacterPathPage() {
  ...
  const handleFindPath = async () => {
    ...
    try {
      ...
      setPath(data.nodes);
      setGraphData(buildPathGraphData(data.nodes));
    } catch (err) {
  ...
  return (
    ...
    <div style={{ display: 'inline-flex', gap: 16, marginBottom: 16 }}>
          ...
          {showFromDropdown && fromSuggestions.length > 0 && (
            <ul className="result-list" style={{ position: 'absolute', zIndex: 10, width: '100%' }}>
              {fromSuggestions.map((s) => (
                <li key={s} style={{ cursor: 'pointer' }} onClick={() => { setFrom(s); setShowFromDropdown(false); }}>
                  {s}
                </li>
              ))}
            </ul>
        ...
        <div ref={toRef} style={{ position: 'relative', marginRight: 16 }}>
          <input
            className="search-box"
            type="text"
          ...
          {showToDropdown && toSuggestions.length > 0 && (
            <ul className="result-list" style={{ position: 'absolute', zIndex: 10, width: '100%' }}>
              {toSuggestions.map((s) => (
                <li key={s} style={{ cursor: 'pointer' }} onClick={() => { setTo(s); setShowToDropdown(false); }}>
                  {s}
                </li>
              ))}
            </ul>
      ...
      {path && path.length > 0 && (
        <>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 8, gap: 4 }}>
            ...
            <button
              className={`tab${viewMode === 'path' ? ' active' : ''}`}
